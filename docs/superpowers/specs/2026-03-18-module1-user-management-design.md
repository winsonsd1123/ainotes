...
- **验证项 2 (全链路打通 - 注册)：** 输入合法凭证注册。验证：展示层 Loading → 业务层 redirect → 数据层 `auth.users` 和 `public.profiles` 均生成新记录；若产品要求成功反馈，则同时验证出现「注册成功」类提示（与 REQ-00A 一致）。
- **验证项 3 (全链路打通 - 登录/异常)：** 使用错误密码登录。验证：业务层捕获异常并返回 error 对象 → 展示层成功解析并弹出 Toast 提示。
- **验证项 4 (全链路打通 - 登录成功)：** 使用已注册且正确的邮箱与密码登录。验证：展示层 Loading → 业务层建立 Session 并 `redirect('/notes')`，进入笔记相关页面（与 REQ-00B 一致）。
- **验证项 5 (登出成功)：** 在已登录状态下于 `/notes`（或任意受保护页）点击退出登录。验证：Session 被清除，页面重定向至 `/login`（与 REQ-00C 一致）。
- **验证项 6 (业务逻辑层防御 - 未登录访问)：** 在完成验证项 5 后（确保未登录），手动在浏览器地址栏访问 `/notes`，验证被 Middleware 拦截并重定向；跳转目标应为 `/login`，且携带 `redirectUrl=/notes`（或项目约定的等价 query），以便登录后回到原目标（与 REQ-04 一致）。
- **验证项 7 (业务逻辑层防御 - 服务端校验)：** 使用合法格式字段但通过 DevTools 等方式篡改表单或构造请求，使 Server Action 收到仍不符合规则的 `FormData`（如过短密码）。验证：`signUpAction` / `signInAction` 在服务端二次校验失败，不写入数据库、不建立 Session，并以约定方式反馈（如字段错误或 Toast），与 §2.2 服务端校验描述一致。